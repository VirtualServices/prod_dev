<?php
	...
	# RSS file name
	define('RSS_FILENAME', 'onorder-test');
	...
	*/
	...
	$image->link = 'http://hip.richland.lib.sc.us/ipac20/ipac.jsp';
	$image->width = 88;
	$image->height = 15;
	$rss->image = $image;
    */
...
from rv_bib_author a11, rv_bib_control a12, rv_item   a13, rv_isbn   a14, rv_title_inverted  a15, collection  a16 where a11.bib_id = a12.bib_id and a11.bib_id = a13.bib_id and a11.bib_id = a14.bib_id and a11.bib_id = a15.bib_id and a13.collection = a16.collection and (a13.collection in ('le')and (a14.processed IS NOT NULL) and a12.creation_date between '03/01/2009' and '05/30/2019') order by a12.creation_date desc");
	...
	if($sdbh != NULL)
	{
		...
		{var_dump($srow);
			...
			$item->description = $srow[1]. ' &nbsp;&nbsp;<i>' . $srow[5] .'</i> &nbsp;&nbsp; Ordered on-' . substr($srow[3], '0', '6') ;
			$rss->addItem($item);
		}
	}
