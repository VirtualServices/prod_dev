<?php
	...
	# RSS file name
	define('RSS_FILENAME', 'checkin-sa');
	...
	$rss->title = 'Most recently checked-in items at the St. Andrews Regional Branch, Richland County Public Library';
	$rss->description = 'Most recently checked-in items at the St Andrews Regional Branch. Click a title to place a hold request.';
	...
	*/
	...
	$image->url = '';
	$image->link = 'http://ipac.sirsidynix.net/ipac20/ipac.jsp';
	$image->width = 88;
	$image->height = 15;
	$rss->image = $image;
    */
...
order by cki_time desc");
	...
	if($sdbh != NULL)
	{
		...
		{var_dump($srow);
			...
			$item->link = 'http://ipac.sirsidynix.net/ipac20/ipac.jsp?profile=int&full=3100001~!' . $srow[1].'~!3';
