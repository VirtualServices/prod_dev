<?php
	...
	# RSS file name
	define('RSS_FILENAME', 'new-available-nonfiction');
	...
	$rss->title = 'Available new nonfiction books at the Richland County Public Library';
	$rss->description = 'Available new nonfiction books at RCPL.  Click on a title to place a hold request on that item.';
	$rss->link = 'http://www.myrcpl.com';
	$rss->lastBuildDate = $date_string;
	$rss->pubDate = $date_string;
	$rss->copyright = 'Copyright 2009-2010, Richland County Public Library';
...
where   a11.bib_id = a12.bib_id and 
        ...
        a13.item_status =  'i' and 
        (a13.collection in ('le')and 
        a12.creation_date between '03/01/2009' and '05/30/2019') and
        a14.processed = (select min(processed) from rv_isbn where bib_id = a15.bib_id)
...
");
	...
	if($sdbh != NULL)
	{
		...
		{var_dump($srow);
			...
			$item->description = '<img src="http://www.syndetics.com/index.aspx?type=xw12&isbn=' . trim($srow[2]) . '/SC.GIF&client=richlandcpubl" align="left"><p>&nbsp;&nbsp;By <a href="http://myrcpl.ipac.sirsidynix.net/ipac20/ipac.jsp?menu=search&aspect=basic_search&npp=20&ipp=20&spp=20&profile=int&ri=1&index=.NW&term=' .$srow[1]. '">' .$srow[1]. '</a></p> &nbsp;&nbsp;<i>' .$srow[6] .'</i><p class="clear"></p>' ;
			$rss->addItem($item);
		}
	}
