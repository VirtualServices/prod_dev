...
sycom('SET ROWCOUNT 50');
	...
	$sdbh = sycom("select convert(varchar(3), (select count(*) from item i where i.bib# = t.bib#)) 'copies', (Select count(*) from request r where r.bib# = t.bib# having count(*)>1) 'reqs', t.bib# 'BIB', t.text 'title', t.bib# from bib t where t.tag='245' and t.bib# in (Select bib# from request) and convert( int, (Select count(*) from item i where i.bib# = t.bib# having count(*)>0)) < convert( int, (Select count(*) from request r where r.bib# = t.bib#))order by Reqs desc");
