<?php
	...
	# RSS file name
	define('RSS_FILENAME', 'onorder-juvenile-dvd');
	...
	$rss->title = 'Most recently ordered juvenile DVDs at the Richland County Public Library';
	$rss->description = 'Most recently ordered juvenile DVDs for our collection.  Some items show duplicates because they have more than one isbn number.  Click on a title to place a hold request on that item.';
...
order by a12.creation_date desc");
	...
	if($sdbh != NULL)
	{
		...
		{var_dump($srow);
			...
			$item->description = '<img src="http://www.syndetics.com/index.aspx?type=xw12&isbn=' . $srow[2] . '/SC.GIF&client=richlandcpubl" align="left"><p>&nbsp;&nbsp; Ordered on-' .substr($srow[3], '0', '6'). '<p class="clear"></p>' ;
			$rss->addItem($item);
		}
	}
